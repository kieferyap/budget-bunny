...
\label{sec:edit-account-screenshots}
...
\screentable{
    ...
    \row{2. Save Button}
    	{Button}
        {This button has the same functionality as the Done Button (See section: \nameref{sec:add-account-screenshots}, Item \#2), but it updates the core data instead of adding a new entry. \doublenewline
        Localization Key: BUTTON\_SAVE
        } 
    \row{3. Current Balance Cell}
    	{Table Cell}
        {This cell functions is similar to the Initial Balance cell in the \nameref{sec:add-account-screenshots} section, but it has its localization key changed. \doublenewline
...
\label{sec:edit-account-use-cases}
